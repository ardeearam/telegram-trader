...
module.exports = (() => {
  class Trader {
    ...
    start() {
      ...
      this.bot.onText(/\/register/, this.registerHelp());
      this.bot.onText(/\/balance/, this.balance());
    }
...
Please do NOT send your API Keys to this group. Kindly delete your API Keys if sent by accident.`);
          ...
          this.bot.sendMessage(chatId, 'Usage: /register <apiKeyId> <apiKeySecret>');
        }
...
`; 
    ...
    balance() {
      return (msg, match) => {
        ...
        if (chatType == "group") {
          this.bot.sendMessage(chatId, `To check your balance, please send me a private message @${from}.`);
          return;
        }
        ...
        if(!this.clients[from]) {
          ...
          return;
            ...
            this.clients.pub.addStream(symbol, 'wallet', data => {this._quote[symbol] = data[data.length - 1]});
            setTimeout(() => {
              resp = `${JSON.stringify(this._quote[symbol])}`  
              this.bot.sendMessage(chatId, resp);
            }, 500);
